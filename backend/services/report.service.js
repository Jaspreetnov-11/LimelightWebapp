...
function toCsv(headers, rows) {
  ...
  return 'FEFF' + [headers.map(sanitize).join(','), ...rows.map(r => r.map(sanitize).join(','))].join('\n');
}
...
class ReportService {
  ...
  async getPaymentsLedgerCsv() {
    const [payments, employees] = await Promise.all([
      paymentModel.findAll({}, { orderBy: 'date DESC' }),
      employeeModel.findAll()
    ]);
    const empMap = Object.fromEntries(employees.map(e => [e.id, e.name]));
    const headers = ['Date', 'Staff Name', 'Payment Type', 'Amount (INR)', 'Note'];
    const rows = payments.map(p => [p.date, empMap[p.emp] || p.emp, p.type, p.amount, p.note || '']);
    return toCsv(headers, rows);
  }
  ...
  async getStaffDirectoryCsv() {
    ...
    const rows = employees.map(e => [e.name, e.emp_id || '', e.role || '', e.dept || '', e.email || '', e.phone || '', e.joined || '', e.salary || 0]);
    return toCsv(headers, rows);
  }

  async getTasksCsv() {
    const [tasks, projects, employees] = await Promise.all([
      taskModel.findAll({}, { orderBy: 'created_at DESC' }),
    ...
    const rows = tasks.map(t => [
      ...
      t.assigned || '', t.deadline || '', t.mins || 0, t.status
    ]);
    return toCsv(headers, rows);
  }
  ...
  async getProjectsCsv() {
    ...
    const rows = projects.map(p => {
      ...
      return [p.name, p.client || '', Number(p.billable) ? 'Yes' : 'No', alloc, consumed, `${pct}%`, p.status];
    });
    return toCsv(headers, rows);
  }
}
