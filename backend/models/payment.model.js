...
class PaymentModel extends BaseModel {
  ...
  filterPayments({ empId, month, type, limit = 500, offset = 0 } = {}) {
    let sql = `
  ...
  getTotalPaidForEmployee(empId, month = null) {
    let sql = `
      SELECT
        COALESCE(SUM(CASE WHEN type = 'Fine' THEN -amount ELSE amount END), 0) as total_paid
      FROM lh_payments
      WHERE emp = ?
    `;
    const params = [empId];
