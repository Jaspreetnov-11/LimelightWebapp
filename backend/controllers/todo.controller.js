...
const getMyTodos = catchAsync(async (req, res) => {
  ...
  const todos = await todoModel.getUserTodos(ownerId);
...
const toggleTodo = catchAsync(async (req, res) => {
  ...
  const toggled = await todoModel.toggle(id, ownerId);
  if (!toggled) {
...
const deleteTodo = catchAsync(async (req, res) => {
  ...
  const existing = await todoModel.findOne({ id, owner: ownerId });
  if (!existing) {
    throw new AppError('Todo not found or not owned by you', 404);
  }
