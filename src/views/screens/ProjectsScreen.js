  ...
  const body = () => {
    ...
    return (<>
      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
        <div className="panel"><div className="panel-h" style={{ fontSize: 16 }}>Project Health</div><div className="health"><Donut parts={[[over ? '#FF5C7A' : pc > 0 ? '#4ADE95' : '#2A2A30', Math.min(100, pc)], ['#2A2A30', Math.max(0, 100 - Math.min(100, pc))]]} center={pc + '%|'} size={110} /><div><Chip tone={over ? 'pk' : pc > 0 ? 'gr' : 'gy'}>{over ? 'Overrun' : pc > 0 ? 'On Track' : 'Not started'}</Chip><div style={{ fontWeight: 600, marginTop: 10 }}>{hm(used)} / {hm(alloc)}</div><small style={{ color: 'var(--muted)' }}>Taken / Allocated · est. {hm(p.est_mins)}</small></div></div></div>
