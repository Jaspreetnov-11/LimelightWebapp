  ...
  const body = () => {
    ...
    return (<>
      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
        <div className="panel"><div className="panel-h" style={{ fontSize: 16 }}>Project Health</div><div className="health"><Donut parts={[[over ? '#FF5C7A' : pc > 0 ? '#4ADE95' : '#2A2A30', Math.min(100, pc)], ['#2A2A30', Math.max(0, 100 - Math.min(100, pc))]]} center={pc + '%|'} size={110} /><div><Chip tone={over ? 'pk' : pc > 0 ? 'gr' : 'gy'}>{over ? 'Overrun' : pc > 0 ? 'On Track' : 'Not started'}</Chip><div style={{ fontWeight: 600, marginTop: 10 }}>{hm(used)} / {hm(alloc)}</div><small style={{ color: 'var(--muted)' }}>Consumed / Allocated</small></div></div></div>
  ...
  return (
    <div className="two">
      <aside><b style={{ fontSize: 14 }}>Projects</b>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}><DateBtn>{new Date().toLocaleDateString('en-GB', { month: 'short', year: 'numeric' })}</DateBtn><button className="tb-btn solid" style={{ height: 34, padding: '0 12px', fontSize: 12.5 }} onClick={() => modals.open('project')}>+ Add Project</button></div>
      ...
      <div className="content">
        {p && (<>
          <div className="panel"><div className="proj-head"><span className="ini">{ini(p.name)}</span><h2>{p.name}</h2><Chip tone="gy">{p.billable ? 'Billable' : 'Non-Billable'}</Chip><Chip tone="pu">Project</Chip><Chip tone={p.status === 'Approved' ? 'gr' : 'or'}>{p.status === 'Approved' ? '✓ ' : ''}{p.status || ''}</Chip>
            <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}><Sq icon="file" label="Edit project" onClick={() => modals.open('project', p.id)} /><Sq label="Delete project" onClick={remove} style={{ color: 'var(--danger)' }}>✕</Sq></div></div>
            <div className="proj-meta"><span>Project Start Date: <b>{fmtDY(p.start)}</b></span><span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>Manager: <Avatar e={d.empById[p.manager]} /> {d.empName(p.manager)}</span><span>Assigned Members: {members.length ? members.map(e => e.name).join(', ') : '—'}</span></div></div>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}><div className="proj-tabs">{[['overview', 'chart', 'Overview'], ['tasks', 'circle-check', 'Tasks'], ['files', 'folder', 'Files'], ['members', 'users', 'Members']].map(([k, ic, l]) => <button key={k} className={view === k ? 'on' : ''} onClick={() => setView(k)}><Icon name={ic} size={14} />{l}</button>)}</div><button className="tb-btn solid" onClick={() => modals.open('task', null, { project: p.id })}>+ Add task</button></div>
        </>)}
        {body()}
      </div>
    </div>
