    ...
    <div className="two">
      ...
      <div className="content">
        ...
        <div className="emp-charts">
          <Panel title={'Attendance · ' + new Date().toLocaleDateString('en-GB', { month: 'short' })} bodyStyle={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}><Donut parts={[['#4ADE95', st.present], ['#FFB84D', st.half], ['#FF5C7A', st.absent], ['#6FA8FF', st.leave], ['#2A2A30', st.unaccounted]]} center={(st.present + st.half) + '|Days'} size={150} /><Legend items={[['#4ADE95', 'Present ' + st.present], ['#FFB84D', 'Half ' + st.half], ['#FF5C7A', 'Absent ' + st.absent], ['#6FA8FF', 'Leave ' + st.leave]]} /></Panel>
