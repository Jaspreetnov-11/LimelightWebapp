...
import { fmtD, inr, monthLabel, PAY_TYPES, shiftMonth, thisMonth } from '@/lib/format';
  ...
  return (
    <div className="content">
      ...
      <div className="panel" style={{ overflowX: 'auto' }}><table><thead><tr><th>Staff</th><th>Salary</th><th>P</th><th>HD</th><th>A</th><th>L</th><th>OT h</th><th>Fine h</th><th style={{ textAlign: 'right' }}>Earned</th><th style={{ textAlign: 'right' }}>Paid</th><th style={{ textAlign: 'right' }}>Pending</th><th></th></tr></thead><tbody>
        {rows.map(r => <tr key={r.employee.id}><td style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Avatar name={r.employee.name} av={r.employee.av} ini={r.employee.ini} /><div><div style={{ fontWeight: 500 }}>{r.employee.name}</div><small style={{ color: 'var(--muted)' }}>{r.employee.empId || ''} · {r.employee.dept || ''}</small></div></td><td className="money">{inr(r.employee.salary)}</td><td>{r.stats.present}</td><td>{r.stats.half}</td><td style={{ color: r.stats.absent ? 'var(--danger)' : undefined }}>{r.stats.absent}</td><td>{r.stats.leave}</td><td>{r.stats.otHours}</td><td>{r.stats.fineHours}</td><td className="money" style={{ textAlign: 'right' }}>{inr(r.earned)}</td><td className="money pos" style={{ textAlign: 'right' }}>{inr(r.paid)}</td><td className={'money ' + (r.pending > 0 ? 'neg' : r.pending < 0 ? 'pos' : '')} style={{ textAlign: 'right' }}>{inr(r.pending)}</td><td style={{ whiteSpace: 'nowrap' }}>{r.pending > 0 ? <DateBtn icon={null} style={{ height: 30 }} onClick={() => modals.open('payment', null, { emp: r.employee.id, amount: Math.round(r.pending), type: 'Salary', after: load })}>Pay {inr(r.pending)}</DateBtn> : <Chip tone="gr">Settled</Chip>}</td></tr>)}
      </tbody></table>{!rows.length && <Empty>{pr ? 'Add staff to run payroll' : 'Loading payroll…'}</Empty>}</div>
    </div>
  );
}
