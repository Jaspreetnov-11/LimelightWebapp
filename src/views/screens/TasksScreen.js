...
export function TasksScreen() {
  ...
  const card = t => { const od = overdue(t); const { acts, lead } = actionsFor(t); return (
    <div className={'tcard' + (dragId === t.id ? ' dragging' : '')} key={t.id} draggable={lead} onDragStart={ev => { if (!lead) { ev.preventDefault(); return; } setDragId(t.id); ev.dataTransfer.effectAllowed = 'move'; try { ev.dataTransfer.setData('text/plain', t.id); } catch (x) { /* ignore */ } }} onDragEnd={() => { setDragId(null); setOverCol(''); }}>
      ...
      <Timer t={t} now={now} />
      <Assignees task={t} />
      {acts.length > 0 && <div className="move">{acts.map(([k, l, cls]) => <button key={k} className={cls} onClick={() => move(t.id, k)}>{l}</button>)}</div>}
    </div>); };
        ...
        <div className="mtask-list">
          {mList.map(t => { const od = overdue(t); const { acts, lead } = actionsFor(t); return (
            <div className={'mtask' + (t.status === 'completed' ? ' done' : od ? ' late' : '')} key={t.id}>
              ...
              <div className="mtask-actions" style={{ flexWrap: 'wrap' }}>
  ...
  return (
    <>
      <div className="task-top">
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}><Seg items={[['board', '▦ Board'], ['list', '☰ List']]} value={view} onChange={setView} /><Sq icon="down" label="Export CSV" onClick={exportCsv} style={{ border: 0 }} />{d.canAssign && <button className="tb-btn solid" style={{ height: 34 }} onClick={() => modals.open('task')}>+ Assign task</button>}</div>
      </div>
      <div className="board-wrap">
        ...
        {view === 'board' ? (
          ...
          <div className="content"><div className="panel">
            <div className="task-row head"><span>Task</span><span>Project</span><span>Assignee</span><span>Assigned</span><span>Deadline</span><span>Est / Taken</span><span>Status</span></div>
            {listPager.items.map(t => <div className="task-row" key={t.id}><LinkBtn onClick={() => (d.isLeaderOf(t.project) ? modals.open('task', t.id) : null)} style={{ textAlign: 'left' }}>{t.title}</LinkBtn><span>{t.project_name || d.projName(t.project)}</span><span>{d.taskAssigneeNames(t)}</span><span>{fmtD(t.assigned)}</span><span style={{ color: overdue(t) ? 'var(--danger)' : undefined }}>{fmtD(t.deadline)}</span><span>{hm(t.mins)} / {hm(takenMins(t, now))}</span><TaskChip status={t.status} /></div>)}
